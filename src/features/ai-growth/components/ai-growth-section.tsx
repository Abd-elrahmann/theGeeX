...
export function AiGrowthSection() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (progress) => {
    syncActiveIndexFromProgress(progress, aiGrowthRows.length, setActiveIndex);
  });
  ...
  return (
    <section
      ...
      className="relative mt-(--ai-growth-margin-top) min-h-(--ai-growth-scroll-height) w-full bg-background px-(--ai-growth-padding-x) py-(--ai-growth-padding-y)"
    >
      <div className="mx-auto grid min-h-(--ai-growth-mobile-stage-height) w-full max-w-(--ai-growth-container-max-width) grid-cols-1 items-start gap-(--ai-growth-grid-gap) lg:sticky lg:top-(--ai-growth-sticky-top) lg:h-(--ai-growth-stage-height) lg:min-h-0 lg:grid-cols-[minmax(var(--ai-growth-grid-one-min-width),1fr)_minmax(var(--ai-growth-grid-two-min-width),1fr)]">
