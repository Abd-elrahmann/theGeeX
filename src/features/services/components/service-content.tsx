...
const serviceContentTitleClassName = cn(
  ...
  "text-[var(--Purple_Light,#2558b4)]",
  "[font-feature-settings:var(--services-content-font-features)]",
);

const serviceContentHeaderHeightClassName = "min-h-[67.2px]";

const serviceContentFeaturesOffsetClassName = "pt-[91.2px]";

const serviceContentDescriptionClassName = cn(
  "absolute left-0 top-[91.2px] m-0 h-auto w-full min-w-0 p-0",
  "whitespace-pre-wrap wrap-break-word break-words",
  "font-poppins text-[16px] leading-[1.6] font-normal not-italic tracking-[0em]",
  "text-[var(--Text_Color,#2c3134)]",
  "[font-feature-settings:'blwf'_on,'cv09'_on,'cv03'_on,'cv04'_on,'cv11'_on,'zero'_on]",
);
    ...
    return (
      <article className={servicesContentContainerClassName}>
        ...
        <div className="service-content-details relative flex min-h-0 w-full flex-1 flex-col">
          {showContentTitle ? (
            <header className="w-full shrink-0 pb-(--services-content-header-desc-gap)">
          ...
          <ul
            className={cn(
              "service-content-description-list static space-y-1",
