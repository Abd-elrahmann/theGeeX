...
function PackageDetailIncludedRow({
  includedItem,
  isLast,
}: {
  includedItem: PackageIncludedItem;
  isLast: boolean;
}) {
  return (
    <div className={cn(
      "flex w-full flex-col gap-(--packages-detail-included-row-gap) pt-(--packages-detail-included-first-row-padding-top) pb-(--packages-detail-included-row-padding-y) first:pt-(--packages-detail-included-row-padding-y) md:w-[98%] md:flex-row md:items-start md:flex-nowrap",
      !isLast && "border-b border-(--packages-detail-included-row-border-color)",
    )}>
    ...
    <section className="flex w-full flex-col gap-(--packages-detail-included-gap)">
      ...
      <div className="flex w-full flex-col gap-(--packages-detail-included-rows-gap)">
        {includedItems.map((includedItem, index) => (
          <PackageDetailIncludedRow
            key={includedItem.title}
            includedItem={includedItem}
            isLast={index === includedItems.length - 1}
          />
        ))}
      </div>
    </section>
